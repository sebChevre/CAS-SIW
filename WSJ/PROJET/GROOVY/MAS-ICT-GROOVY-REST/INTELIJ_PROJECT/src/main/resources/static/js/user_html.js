...
var displayUsersList = function () {
	...
	$.get( "/usersList", {
		...
		for(var user in jsonData.data.users){
			...
			+ '<button type="button" class="btn_add_track" data-user-id="' + u._id +'" class="btn btn-default">Valider</button></td></tr>');
		}
...
var displayTracksForUser = function ($element,userId) {
	...
	$.get( "/userTracks/" + userId, {
		...
		var tracksTable = '<tr class="user_tracks_' + userId + '"><td colspan="3"><table  class="table table-bordered "><thead><tr>'+
		'<th>Nom</th>'+
		'<th>Description</th>'+
		'<th>Date</th>'+
		'</tr>'+
		'</thead>'+
		'<tbody id="tracksResultZone">'


		for(var track in jsonData.data.tracks){
			var t = jsonData.data.tracks[track];
			tracksTable += '<tr class="user_tracks_' + userId +'"><td><a href="/track/' + t._id +'.json">'+ t.name +'</a></td><td>' + t.description +'</td><td>' + t.date +'</td></tr>';

			console.log(t);
		}

		$element.attr('data-display','true');
		tracksTable.concat('</tbody></table></tr></td>')
		$('#user_' + userId).after(tracksTable);
	});
}